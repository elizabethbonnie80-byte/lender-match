...
- [x] **A-4 — Lender logos: in colour, bigger, and remove the heading above them.** *"They look good on a
...
- [x] **A-5 — Remove the sub-heading under "Welcome Back"** (`Sign in to access your secure lending portal.`)
- [x] **A-30 — On sign-up, remove the grey line** (`Join our secure lending ecosystem and streamline your
      financial operations.`) **and the icons next to Broker and Lender.**

## A2) Lender — New Deals

- [x] **A-1 — Dwelling type as CHECKBOXES in the lender filters.** *"We need to be able to exclude certain
...
- [x] **A-6 — Remove the verbiage under New Deals** (`Recently submitted deals — sorted by closing date.
      COF-specified deals appear first.`)
- [x] **A-7 — Hide the result-count line** (`2 deals total · sorted by closing date (COF priority)`).
      *"hide this for now? We may reinstate it later"* → **hide, don't delete.**
- [x] **A-11 — Remove the note at the bottom of New Deals** (`Broker contact details, client name, and
      full property addresses are not disclosed at this stage.`)
- [x] **A-12 — Decline: fix the all-red button + use this exact copy:**
...
- [x] **A-13 — A declined deal must no longer show up in the lender's deals.** `decline_deal` already
      ...
      the two must agree.
- [x] **A-14 — Occupancy is not visible to lenders. ✅ CONFIRMED BUG.** `deals.occupancy` exists and is
...
- [x] **A-15 — A max GDS/TDS filter wrongly drops deals with no GDS/TDS.** *"it removed the deal without
...
- [x] **A-8 — Remove two lines from the offer dialog:** `Commission is entered in basis points (bps). Your
...
- [x] **A-10 — Replace the platform-fee fine print with exactly:**
...
- [x] **A-16 — A message sent from Submitted Offers never reached the messages record. ✅ REAL BUG,
...
- [x] **A-17 — *"please ensure the messages are not deleted until 120 days after the chosen closing date
      ...
      to then not delete.
- [x] **A-18 — Remove the grey line under "Messages"** (`Your deal conversations. The broker's identity
      stays hidden until your offer is accepted.`)

## A5) Lender — Submitted Offers

- [x] **A-19 — The lender can't see the loan details; should see the same data as New Deals.** Reuse
      `components/lender-deal-sections.tsx`. Pairs with A-14.
- [x] **A-20 — Remove the "DECLINED" counter** (*"We don't want to highlight that"*). = B-35.
- [x] **A-21 — Remove the verbiage** (`Deals where you have sent an offer — track status and broker
      responses in real time.`)
- [x] **A-22 — Withdraw offer: the confirm button is solid red with NO visible label.** Same root cause as
...
- [x] **A-23 — Replace the intro copy with exactly:**
...
- [x] **A-24 — Remove an element from the invoices page** (*"Lenders will be paying individually so it's
...
- [x] **A-26 — Remove the grey line under "Invoices"** (`Lender fee invoices for funded deals`).

## A7) Lender — FAQ / Contact

- [x] **A-27 — Remove the grey line under "Lender FAQ's"** (`Answers to the most common questions from
      lenders on our platform.`)
- [x] **A-28 — Support hours → exactly** `Monday to Friday 9:00 AM - 5:00 PM MST.` (currently *"Monday to
...
- [x] **A-29 — Remove the grey line under "Contact Us"** (`Our lender support team is here to help. Reach
...
- [x] **B-1 — Phone numbers submitted in messages were NOT flagged in the admin portal.** Anti-contact
...
- [x] **B-2 — No way to mark an invoice paid in Admin, so the analytics can never show paid.** ✅ Verified
...
- [x] **B-5 — Income label → exactly** `Casual/seasonal/part-time income (2y avg)` (*"what it was before"*).
- [x] **B-6 — Credit issues: append "(non-mortgage)" to the three late-payment options** → `30+ day lates
      (non-mortgage)`, `60+ day lates (non-mortgage)`, `90+ day lates (non-mortgage)`.
- [x] **B-7 — Remove the grey line under "Create New Deal"** (`Complete all sections to submit your
      mortgage deal for processing.`)
- [x] **B-8 — Reorder the program checkboxes** so the two co-signer options and the guarantor sit
...
- [x] **B-9 — Rename "Credit Score" → "Primary Credit Score"** on page 3 of Create Deal (*"As it was in
      bubble"*).
- [x] **B-36 — Add to the bottom of the LAST page of Create Deal, exactly:**
...
- [x] **B-34 — The document-scan notice must not mention the invoice.** *"I like that this pops up - so
...
- [x] **B-10 — Remove the grey line under "Deal Room"** (`Manage and track all your mortgage deals`).
- [x] **B-23 — Mirror to the broker portal everything added on the lender side** — *"the titles showing,
      the product choices etc."* (i.e. the same missing-fields work as A-14/B-12…41, applied to the
      broker's deal view).
- [x] **B-24 — A received offer shows the broker the FULL commission, not what they will receive.**
...
- [x] **B-26 — Replace the accept-offer confirmation copy with exactly:**
...
- [x] **B-31 — Delete the post-accept green banner** (`The lender has been notified and the platform-fee
...
- [x] **B-32 — Instead, show a switch counter per calendar month**, as Bubble had.
      ✅ The data exists — `profiles.offer_switches_this_month` + the monthly reset cron.
- [x] **B-38 — Add near the top of lender New Deals AND Maturing Deals** (visible on load but not
      conspicuous), exactly:
...
- [x] **B-37 — Add when a lender sends an offer, exactly:**
...
- [x] **B-40 — Fine print on NON-prequal offers received** (not a pop-up — *"just add it in below/inside
      each offer"*):
...
- [x] **B-11 — Hide the "1 new this week" badge in the lender deal room** (*"I didn't see it before. We may
      want it showing at some point but not now"*) → hide, don't delete.
- [x] **B-12 — The product/program options are gone**: FTHB, Networth, Medical Professional etc. must show.
- [x] **B-13 — Liquid and non-liquid assets are not showing for lenders.**
- [x] **B-14 — Show the number of door titles, and add a filter for it.** (We added `door_titles_count` in
      migration 36 and a `max_door_titles` filter in 43 — so verify what is actually missing: the display,
      the filter, or both.)
- [x] **B-15 — The TransUnion checkbox result must show on the lender side.**
- [x] **B-41 — Also missing: transaction purpose, transaction type, and the property options** (New Build,
      Hobby Farm etc.) — *"please add all of those so they are visible for lenders and brokers"*.

## B5) Lender — filters

- [x] **B-18 — Max Doors removes deals with 0 doors.** *"It should only remove deals that are OVER the
      number of doors allowed."*
- [x] **B-19 — Max Titles: same bug.**
- [x] **B-20 — Max Acres: same bug** — *"it is removing deals that don't have an amount listed. This is not
      what we want."*
      → **B-18/19/20 + A-15 are one fix**: a max filter must pass rows whose value is NULL or 0.
- [x] **B-21 — Remove "Married / Common Law" as a filter** (*"We don't need this as a filter"*).

## B6) Lender — offers & invoices

- [x] **B-22 — With product "Open" the deduction shows 5 bps; it must be 3 bps.** ✅ Verified:
...
- [x] **B-27 — Invoice amount rounds up: should read $187.50, shows $188.** ✅ Verified: the DB stores
...
- [x] **B-28 — Lender invoices: keep only Pending and Overdue.** *"We don't need a tally of funds due or a
      tally of paid invoices. We need these in the admin section though, just not in the lender section."*
      = the spec for **A-24**.
- [x] **B-35 — Remove the declined tally in lender Submitted Offers.** = **A-20**.
...
| --- | --- |
| Implemented | 54 |
| Investigated → no code needed (A-13, A-17, B-1) | 3 |
| Waiting on the client | 8 (A-2, A-3, A-9, A-25, B-25, B-29, B-33, B-39) |
| Out of Round 3 scope, to quote (B-3, B-4, B-16, B-17, B-30) | 5 |

Migrations added: **52** (prequal lender window + document retention), **53** (28 lender-visible deal
fields), **54** (null-safe max filters), **55** (Open term = 3 bps), **56** (dwelling-type exclusions).
Gate green throughout: 0 type/lint errors, EN/FR parity, 23/23 smokes.

### Findings from the two items that needed reproducing

**A-13 — a declined deal reappearing: could not reproduce, and it is already covered.** After
`decline_deal` the deal is gone from the New Deals feed, the Maturing feed, the ad-hoc filtered feed,
the messages inbox, and the raw `deals` row is no longer readable at all. One case does legitimately
remain visible: if the lender had already made an offer, the deal stays in **Submitted Offers** — which
is correct, they have a live offer on it. Worth confirming that is not what he saw.
⚠️ Note this now interacts with A-12's new copy, which promises "You will not be able to view any deals
you have declined."

**B-1 — phone numbers not flagged in the admin portal: the alerting works; the message path he used did
not.** Verified end to end: `scan_and_log` detects the number and writes the `admin_alerts` row,
`send_deal_message` blocks at the data layer, and an admin reads the row through the exact query the
Alerts page uses. The explanation is **A-16**: the Submitted Offers "Message broker" button was a
prototype that never called the scanner at all, so nothing was ever flagged from there — and he says as
much himself in B-16 ("I think maybe it wasn't working in the submitted offers tab yesterday").
**Fixed by the A-16 wiring; no separate work needed.**

**A-17 — messages disappearing after an hour: same root cause.** Nothing in this codebase deletes
messages, so those sends were never persisted. Also closed by A-16.
