-- Round 4 (approved for this item only, 2026-08-25) — J-2: "HoldCo on title" checkbox (Property
-- section). Scope is deliberately limited to the checkbox + deal display + lender filter/exclusion
-- criterion — the document-checklist rules mentioned in the original Round 4 proposal are NOT part
-- of this migration.
--
-- Follows the existing well_water / septic architecture exactly: a plain boolean flag on `deals`,
-- a matching nullable `exclude_*` boolean on `saved_filters`, enforced as a filter-only predicate in
-- `saved_filter_matches` (never scored — see match_percentage, which this migration does not touch),
-- and threaded through the four lender feed RPCs so New Deals and Maturing Deals (both the
-- saved-filter-chip path and the ad-hoc Filters-panel path) pick it up identically.
--
-- New OUT columns change the RETURNS TABLE shape of the four feed RPCs, which CREATE OR REPLACE
-- cannot do — same DROP + CREATE pattern as migrations 29/31/37/49/53. saved_filter_matches returns
-- boolean (unchanged), so it stays a plain CREATE OR REPLACE.

alter table deals
  add column holdco_on_title boolean not null default false;

alter table saved_filters
  add column exclude_holdco_on_title boolean;

comment on column deals.holdco_on_title is
  'Round 4 J-2 (2026-08-25): a holding company is on title. Plain display flag, same shape as well_water/septic — no document-checklist gating yet.';
comment on column saved_filters.exclude_holdco_on_title is
  'Lender does NOT want deals where a HoldCo is on title. Filter-only — never part of match_percentage.';
...
-- ============================================================================
-- The four feed RPCs: add holdco_on_title to the OUT column list (RETURNS TABLE shape changes,
-- so each is DROPped by its current exact signature before being recreated).
-- ============================================================================
