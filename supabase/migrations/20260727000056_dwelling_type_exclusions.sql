-- Client revision 2026-07-23 (A-1): "Can you change dwelling type to checkboxes please in the lender
-- filters? We need to be able to exclude certain types."
--
-- The filter held ONE dwelling type as an include ("show me only Detached"). The client wants the
-- opposite shape: tick the types you do NOT want. That is the same pattern the income-type and
-- residency-status criteria already use, so this adds a `dwelling_types_excluded` array and enforces it
-- in saved_filter_matches — which means the saved-filter chips, the ad-hoc Filters panel and the
-- filter-match notifications all pick it up from one place.
--
-- The old single-value `saved_filters.dwelling_type` column and its predicate are KEPT, not dropped:
-- any filter a lender already saved with it set keeps behaving exactly as they set it. The UI no longer
-- offers that control, so nothing new can be written to it. Same "retire, never delete" reasoning used
-- for brokerages and the withdrawn dwelling enum values.
--
-- Null-safe by the same rule as migration 54: a deal with no dwelling type recorded is not one of the
-- excluded types, so an exclusion list must not hide it.

alter table saved_filters add column if not exists dwelling_types_excluded dwelling_type[];

comment on column saved_filters.dwelling_types_excluded is
  'Dwelling types the lender does NOT want (A-1, 2026-07-23). Supersedes the single-value dwelling_type include, which is kept only so pre-existing saved filters keep working.';
