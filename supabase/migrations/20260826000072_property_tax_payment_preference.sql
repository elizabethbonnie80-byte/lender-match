-- Property Tax Payment Preference (approved 2026-08-26) — Broker Create Deal, Property section,
-- Property Characteristics: "Lender to pay property taxes" / "Borrower to pay property taxes".
--
-- Follows the holdco_on_title architecture exactly: two plain boolean flags on `deals`
-- (PROPERTY_FLAG_TABLE), two matching nullable `exclude_*` booleans on `saved_filters`, enforced as
-- filter-only predicates in `saved_filter_matches` (never scored — match_percentage is untouched),
-- and threaded through the four lender feed RPCs.
--
-- Mutual exclusivity ("Lender to pay" vs "Borrower to pay") is Create Deal UI behaviour only
-- (app/(broker)/create-deal/page.tsx clears one when the other is checked) — deliberately NOT a
-- database CHECK constraint, matching the existing unenforced precedent for this exact class of
-- opposite-meaning checkbox pair (cosignor_occupying / cosignor_not_occupying has no DB-level
-- mutual-exclusion either). Both columns can independently be true or false at the DB layer; the
-- application is what keeps them from both being true in practice.
--
-- Built from migration 68's (Spousal Buyout/Refinance) saved_filter_matches body — its current
-- latest definition — and migration 71's (TDS) four feed RPC bodies — their current latest
-- definition, which already carries holdco_on_title, spousal_buyout, refinance_plus_improvements,
-- and tds_includes_child_support_alimony forward. All of those are preserved untouched here.

alter table deals
  add column lender_to_pay_property_taxes boolean not null default false,
  add column borrower_to_pay_property_taxes boolean not null default false;

alter table saved_filters
  add column exclude_lender_to_pay_property_taxes boolean,
  add column exclude_borrower_to_pay_property_taxes boolean;

comment on column deals.lender_to_pay_property_taxes is
  'Property Tax Payment Preference (2026-08-26): client wants the lender to pay/remit property taxes. Mutually exclusive with borrower_to_pay_property_taxes in the Create Deal UI only — no DB constraint. Filter-only, never part of match_percentage.';
comment on column deals.borrower_to_pay_property_taxes is
  'Property Tax Payment Preference (2026-08-26): client wants the borrower to pay/remit property taxes. Mutually exclusive with lender_to_pay_property_taxes in the Create Deal UI only — no DB constraint. Filter-only, never part of match_percentage.';
comment on column saved_filters.exclude_lender_to_pay_property_taxes is
  'Lender does NOT want deals where the lender is expected to pay property taxes. Filter-only — never part of match_percentage.';
comment on column saved_filters.exclude_borrower_to_pay_property_taxes is
  'Lender does NOT want deals where the borrower is expected to pay property taxes. Filter-only — never part of match_percentage.';

-- ============================================================================
-- saved_filter_matches: two new lines in the exclusion chain (return type unchanged)
-- ============================================================================
...
-- ============================================================================
-- The four feed RPCs: add lender_to_pay_property_taxes + borrower_to_pay_property_taxes to the OUT
-- column list. tds_includes_child_support_alimony, holdco_on_title, spousal_buyout, and
-- refinance_plus_improvements are carried forward untouched.
-- ============================================================================
