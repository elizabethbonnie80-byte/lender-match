-- Round 4 (approved for this item only, 2026-08-25) — J-3: "Spousal Buyout" + "Refinance Plus
-- Improvements" checkboxes, Deal Information section. Scope is limited to the checkbox + deal
-- display + lender filter/exclusion criterion — the document-checklist rules mentioned in the
-- original Round 4 proposal are NOT part of this migration.
--
-- Both are plain independent boolean flags, unconditional on transaction purpose/type — verified
-- against the current Create Deal wizard, which gates NO existing DEAL_INFO_FLAG_TABLE checkbox
-- (including the naming-closest precedent, purchase_plus_improvements) on purpose/type. Follows the
-- reverse_mortgage architecture exactly: a boolean flag on `deals` (DEAL_INFO_FLAG_TABLE, not
-- PROPERTY_FLAG_TABLE), a matching nullable `exclude_*` boolean on `saved_filters` (nullable/no
-- default, matching the HoldCo migration's choice, not the newer not-null-default-false style used
-- for exclude_reverse_mortgage itself), enforced as a filter-only predicate in `saved_filter_matches`
-- (never scored — match_percentage is untouched), and threaded through the four lender feed RPCs.
--
-- Built from migration 67's (HoldCo) function bodies — the current latest definition of all five
-- functions touched here — so holdco_on_title's OUT columns, SELECT columns, and v_sf assignments
-- are carried forward untouched rather than being accidentally dropped.
--
-- Two independent, non-interacting boolean columns bundled in one migration: safe, and cheaper than
-- two separate DROP+CREATE passes over the four feed RPCs.

alter table deals
  add column spousal_buyout boolean not null default false,
  add column refinance_plus_improvements boolean not null default false;

alter table saved_filters
  add column exclude_spousal_buyout boolean,
  add column exclude_refinance_plus_improvements boolean;

comment on column deals.spousal_buyout is
  'Round 4 J-3 (2026-08-25): a spousal buyout is part of the deal. Plain display flag, unconditional on transaction purpose — same shape as reverse_mortgage.';
comment on column deals.refinance_plus_improvements is
  'Round 4 J-3 (2026-08-25): refinance plus improvements. Plain display flag, unconditional on transaction purpose despite the name — no purpose-gating exists anywhere in this wizard today.';
comment on column saved_filters.exclude_spousal_buyout is
  'Lender does NOT want deals with a spousal buyout. Filter-only — never part of match_percentage.';
comment on column saved_filters.exclude_refinance_plus_improvements is
  'Lender does NOT want deals with refinance plus improvements. Filter-only — never part of match_percentage.';
...
-- ============================================================================
-- The four feed RPCs: add spousal_buyout + refinance_plus_improvements to the OUT column list
-- (RETURNS TABLE shape changes, so each is DROPped by its current exact signature before being
-- recreated). holdco_on_title stays exactly where migration 67 put it.
-- ============================================================================
